    ...
    return (
      <div className="stage-view model-view">
        ...
        <div className="model-workspace">
          <aside className="model-tree"><div className="tree-heading"><div><span className="micro-label">Course hierarchy</span><strong>{model.modules.length} modules · {subtopics.length} subtopics</strong></div></div>{model.modules.map((module) => <div className="tree-module" key={module.id}><div className="module-row"><div className="module-copy" title={`Module reference: ${module.id}`}><strong>{module.title}</strong></div></div><div className="subtopic-tree">{module.subtopics.map((subtopic) => <button key={subtopic.id} className={selected?.id === subtopic.id ? "active" : ""} onClick={() => setSelectedId(subtopic.id)}><span className="tree-sequence">{String(subtopic.order).padStart(2, "0")}</span><span className="tree-item-copy"><strong>{subtopic.title}</strong><small>{subtopic.approvedSourceIds.length} source{subtopic.approvedSourceIds.length === 1 ? "" : "s"} · {subtopic.coverageRequirements.length} requirement{subtopic.coverageRequirements.length === 1 ? "" : "s"}</small></span></button>)}</div></div>)}</aside>
          {selected ? <section className="model-detail"><header className="model-detail-head"><div className="model-detail-title" title={`Subtopic reference: ${selected.id}`}><h2>{selected.title}</h2><p>{selected.purpose}</p></div></header><dl className="model-metadata"><div><dt>Prerequisites</dt><dd>{selected.prerequisiteSubtopicIds.map((id) => subtopics.find((item) => item.id === id)?.title ?? id).join(", ") || "None"}</dd></div><div><dt>Approved sources</dt><dd>{selected.approvedSourceIds.length}</dd></div><div><dt>Coverage</dt><dd>{selected.coverageRequirements.length}</dd></div></dl><section className="detail-section"><h3>Scope contract</h3><p><strong>In scope:</strong> {selected.inScope.join(", ")}</p><p><strong>Out of scope:</strong> {selected.outOfScope.join(", ")}</p></section><section className="detail-section"><h3>Concepts</h3>{selected.concepts.map((concept) => <p key={concept.id} title={`Concept reference: ${concept.id}`}><strong>{concept.name}</strong> — {concept.summary}</p>)}</section><section className="detail-section"><h3>Coverage requirements</h3>{selected.coverageRequirements.map((coverage) => <p key={coverage.id} title={`Coverage reference: ${coverage.id}`}>{coverage.statement}</p>)}</section></section> : null}
        </div>
      </div>
    );
  }
