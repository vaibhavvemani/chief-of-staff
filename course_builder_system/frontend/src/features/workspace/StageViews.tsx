...
function VerificationDetail({ asset, claim, canRevise, canRepair, repairUnavailableReason, onAction }: { asset: ContentAsset; claim?: Claim; canRevise: boolean; canRepair: boolean; repairUnavailableReason?: string; onAction?: (action: string, asset: ContentAsset, claim?: Claim) => void }) {
...
function ContentView({ workspace, canReview, canRevise, canRepair, repairUnavailableReason, repairBusy, onContentAction, onRepairDecision, onRepairRoute }: { workspace: Workspace; canReview: boolean; canRevise: boolean; canRepair: boolean; repairUnavailableReason?: string; repairBusy: boolean; onContentAction?: (action: string, asset: ContentAsset, claim?: Claim) => void; onRepairDecision?: (entry: SourceRepairEntry, candidateId: string) => void; onRepairRoute?: (entry: SourceRepairEntry) => void }) {
  ...
  return (
    <div className="stage-view content-stage">
      ...
      <SourceRepairQueue entries={workspace.sourceRepairs} busy={repairBusy} onDecision={onRepairDecision} onRoute={onRepairRoute} />
      {workspace.sourceRepairs.some((entry) => entry.status === "awaiting_content_repair") ? <div className="repair-routed source-repair-ready"><span aria-hidden="true">✓</span><div><strong>Source approved and route committed atomically</strong><p>Affected asset: {workspace.sourceRepairs.filter((entry) => entry.status === "awaiting_content_repair").flatMap((entry) => entry.affectedAssetIds).join(", ")}. Targeted content repair is ready for NC-80; it has not started yet.</p></div></div> : null}
      ...
      <div className="content-workspace">
        ...
        {selected ? <VerificationDetail asset={selected} claim={selectedClaim} canRevise={canRevise} canRepair={canRepair} repairUnavailableReason={repairUnavailableReason} onAction={onContentAction} /> : null}
      </div>
