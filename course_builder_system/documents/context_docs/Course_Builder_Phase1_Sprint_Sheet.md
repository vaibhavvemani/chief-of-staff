...
| **3 — Verify & measure** | 3 | A separate step fact-checks every claim against its source; we score the output vs the manual and improve it until it matches or beats it. | Verified content + a scorecard showing the 5 core assets at/above manual quality. | Build the scoring/comparison tool. | Build the fact-checking step. *(Both: the improve-and-re-measure loop.)* |
