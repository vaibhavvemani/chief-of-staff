...
- **Current state:** The four-week pipeline prototype and the local Course Builder Studio frontend are implemented and independently reviewed through NC-100, including scoped live revisions NC-303, NC-406, NC-504, and NC-604. The registry makes deterministic/live ownership explicit; live proposals are bounded, schema-validated, reduced through existing domain contracts, and fail without fallback. The browser now exposes real progress, persisted safe activity and diagnostics, provider readiness, actual Markdown Package content, actionable release blockers, and acceptance-path accessibility coverage. The reusable contracts, prompts, context builder, verification, integrity path, FastAPI adapter, React workspace, durable guided intake, typed stage editors and diffs, local acceptance course, live-run evidence, and FRM benchmark are in place. FRM-specific fixtures remain benchmark data, not reusable prompt or contract assumptions.
...
- **Immediate next action:** begin NC-110 with NC-1101 and follow its dependency order. Run credentialed live end-to-end browser acceptance at NC-1104 only after the deterministic, recovery, and negative browser prerequisites are evidenced.
